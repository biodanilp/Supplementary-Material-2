library(deSolve)
library(ggplot2)
state1<-c(y=57.741) # Estado inicial#
times1<-seq(0,6,1/10000) #6 dias con 10000 intervalos por día#
parameters1<-c(Y1=56.806,Y2=96.144) #vector con los valores de los parametros#
#Camu et al., 2007 Ghana <- Ghana - MT1#
...
ODE1<-function(times,state,parameters){ 
  ...
  dy<--(Y1*U1/0.5)*y-(Y2*U2/0.5)*y #ecuación 
  res<-dy
  list(res)
}
...
d <- ggplot() +
  ...
  ggtitle("Ghana - MT1") +
  annotate("text", x = 4, y = 55, label = paste0("R² = ", round(R2, 4), ", RECM = ", round(RECM, 4), ", p-valor =", round(p_value, 4))) +
  ...
  scale_y_continuous(limits = c(0, 58), breaks = seq(0, 58, 10))   # Límites del eje y y los intervalos entre marcas
