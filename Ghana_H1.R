...
state1<-c(y=51.963) # Initial state #
times1<-seq(0,6,1/10000) #6 days with 10000 intervals per day#
parameters1<-c(Y1=50.757,Y2=85.905) #Parameter values#
...
ODE1<-function(times,state,parameters){ 
  ...
  dy<--(Y1*U1/0.4)*y-(Y2*U2/0.4)*y #equation
  res<-dy
  list(res)
}
...
glu<-c(51.963,43.884,24.25,26.098,11.871,4.607,1.714,4.821,5.409,3.701,3.696,1.544,4.428,1.086,2.337,2.837,3.486)
...
d <- ggplot() +
  ...
  xlab("Time (days)") + ylab("Glucose (mg/g)") +
  ggtitle("Ghana - H1") +
  annotate("text", x = 3.2, y = 55, label = paste0("R² = ", round(R2, 4), ", RMSE = ", round(RMSE, 4), ", p-value =", round(p_value, 4), ", D =", round(D, 4))) +
  ...
  scale_y_continuous(limits = c(0, 58), breaks = seq(0, 58, 10))   # y-axis boundaries and intervals between mark
