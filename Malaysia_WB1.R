...
state1<-c(y=39.826) #Initial state #
times1<-seq(0,5,1/10000) #5 days with 10000 intervals per day#
parameters1<-c(Y1=147.56,Y2=66.563) #Parameter values#
...
ODE1<-function(times,state,parameters){ 
  ...
  dy<--(Y1*U1/0.15)*y-(Y2*U2/0.15)*y #equation 
  res<-dy
  list(res)
}
...
glu<-c(39.826,39.926,34.367,31.39,1.613,1.117,0.422,0.918,0.223,1.414,0.72,1.315,1.712)
