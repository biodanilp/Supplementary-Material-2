...
state1<-c(y=67.249) #Initial state #
times1<-seq(0,6,1/10000) #6 days with 10000 intervals per day#
parameters1<-c(Y1=239.00,Y2=83.265) #Parameter values#
#Papalexandratou et al., 2011 Brazil <- Brazil - WB2#
...
ODE1<-function(times,state,parameters){ 
  ...
  dy<--(Y1*U1/0.18)*y-(Y2*U2/0.18)*y #equation 
  res<-dy
  list(res)
}
...
fru<-c(67.249,21.349,14.478,10.974,2.641,0.889,4.057,1.756,1.467,3.812,8.351,9.416,9.081,9.109)
...
d <- ggplot() +
  ...
  xlab("Time (days)") + ylab("Fructose (mg/g)") +
  ggtitle("Brazil - WB2") +
  annotate("text", x = 4, y = 69, label = paste0("R² = ", round(R2, 4), ", RMSE = ", round(RMSE, 4), ", p-value =", round(p_value, 4), ", D =", round(D, 4))) +
  ...
  scale_y_continuous(limits = c(0, 70), breaks = seq(0, 70, 10))   # y-axis boundaries and intervals between marks
