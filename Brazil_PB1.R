...
state1<-c(y=102.342) # Initial state #
times1<-seq(0.5,6,1/10000) #6 days with 10000 intervals per day#
parameters1<-c(Y1=69.333,Y2=145.37) #Parameter values#
#Pereira et al., 2012 Brazil <- Brazil - PB1#
...
ODE1<-function(times,state,parameters){ 
  ...
  dy<--(Y1*U1/0.098)*y-(Y2*U2/0.098)*y #equation 
  res<-dy
  list(res)
}
...
glu<-c(102.342,23.407,1.09,5.923,2.81,9.85,11.593,13.556,10.443,11.303,10.948,1.986)
...
d <- ggplot() +
  ...
  xlab("Time (days)") + ylab("Glucose (mg/g)") +
  ggtitle("Brazil - PB1") +
  annotate("text", x = 4, y = 100, label = paste0("R² = ", round(R2, 4), ", MSE = ", round(MSE, 4)))
...
d <- ggplot() +
  ...
  xlab("Time (days)") + ylab("Glucose (mg/g)") +
  ggtitle("Brazil - PB1") +
  annotate("text", x = 3.5, y = 100, label = paste0("R² = ", round(R2, 4), ", RMSE = ", round(RMSE, 4), ", p-value =", round(p_value, 4), ", D =", round(D, 4))) +
  ...
  scale_y_continuous(limits = c(0, 105), breaks = seq(0, 105, 10))  # y-axis boundaries and intervals between mark
