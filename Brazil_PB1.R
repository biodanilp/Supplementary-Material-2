...
state1<-c(y=95.058) #Initial state#
times1<-seq(0.5,6,1/10000) #6 days with 10000 intervals per day#
parameters1<-c(Y1=62.085,Y2=130.17) #Parameter values#
...
ODE1<-function(times,state,parameters){ 
  ...
  dy<--(Y1*U1/0.12)*y-(Y2*U2/0.12)*y #equation 
  res<-dy
  list(res)
}
...
fru<-c(95.058,24.952,10.14,11.331,6.121,6.98,6.516,13.556,15.189,7.992,6.866,1.324)
...
d <- ggplot() +
  ...
  xlab("Time (days)") + ylab("Fructose (mg/g)") +
  ggtitle("Brazil - PB1") +
  annotate("text", x = 4, y = 100, label = paste0("R² = ", round(R2, 4), ", RMSE = ", round(RMSE, 4), ", p-value =", round(p_value, 4), ", D =", round(D, 4))) +
  ...
  scale_y_continuous(limits = c(0, 105), breaks = seq(0, 105, 10))   # y-axis boundaries and intervals between marks
