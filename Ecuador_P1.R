...
state1<-c(y=45.930) # Initial state #
times1<-seq(0,4,1/10000) #4 days with 10000 intervals per day#
parameters1<-c(Y1=64.395,Y2=85.180) #Parameter values#
#Papalexandratou et al., 2011 Ecuador <- Ecuador - P1#
...
ODE1<-function(times,state,parameters){ 
  ...
  dy<--(Y1*U1/0.05)*y-(Y2*U2/0.05)*y #equation
  res<-dy
  list(res)
}
...
glu<-c(45.93,49.119,47.058,36.219,18.238,6.712,14.375,4.655,3.541,7.934,3.378,0,0.805,2.018,1.769)
