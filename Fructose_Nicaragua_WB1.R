...
state1<-c(y=27.875) #Initial state#
times1<-seq(0,5,1/10000) #5 days with 10000 intervals per day#
parameters1<-c(Y1=21.251,Y2=1450.8) #Parameter values#
#Papalexandratou et al., 2019 Nicaragua <- Nicaragua - WB1#
...
ODE1<-function(times,state,parameters){ 
  ...
  dy<--(Y1*U1/20)*y-(Y2*U2/20)*y #equation 
  res<-dy
  list(res)
}
...
ts = c(0,24/24,48/24,72/24,96/24,120/24)
fru<-c(27.875,31.25,9.999,11.037,12.359,8.667)
