library(deSolve)
library(ggplot2)
state1<-c(y=53.899) # Estado inicial#
times1<-seq(0,4,1/10000) #4 dias con 10000 intervalos por día#
parameters1<-c(Y1=60.784,Y2=82.225) #vector con los valores de los parametros#
#Papalexandratou et al., 2011 Ecuador <- Ecuador - CM1#
...
ODE1<-function(times,state,parameters){ 
  ...
  dy<--(Y1*U1/0.16)*y-(Y2*U2/0.16)*y #ecuación 
  res<-dy
  list(res)
}
...
fru<-c(53.899,43.799,36.867,26.082,30.623,29.513,3.401,4.432,12.484,3.839,8.209,9.754,2.393,1.971,2.749)
