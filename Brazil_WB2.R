...
state1<-c(y=42.936) # Initial state #
times1<-seq(0,6,1/10000) #6 days with 10000 intervals per day#
parameters1<-c(Y1=146.94,Y2=51.194) #Parameter values#
...
ODE1<-function(times,state,parameters){ 
  ...
  dy<--(Y1*U1/0.10)*y-(Y2*U2/0.10)*y #equation
  res<-dy
  list(res)
}
...
glu<-c(42.936,23.452,8.264,4.85,1.27,1.62,2.32,0.294,1.467,1.892,5.975,7.863,6.521,7.19)
...
d <- ggplot() +
  ...
  xlab("Time (days)") + ylab("Glucose (mg/g))") +
  ggtitle("Brazil - WB2") +
  annotate("text", x = 2, y = 50, label = paste0("R² = ", round(R2, 4), ", MSE = ", round(MSE, 4),", RMSE =", round(RMSE, 4)))
...
d <- ggplot() +
  ...
  xlab("Time (days)") + ylab("Glucose (mg/g)") +
  ggtitle("Brazil - WB2") +
  annotate("text", x = 3.2, y = 50, label = paste0("R² = ", round(R2, 4), ", RMSE = ", round(RMSE, 4), ", p-value =", round(p_value, 4), ", D =", round(D, 4))) +
