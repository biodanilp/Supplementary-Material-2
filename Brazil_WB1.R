...
state1<-c(y=55.482) # Initial state #
times1<-seq(0,6,1/10000) #6 days with 10000 intervals per day#
parameters1<-c(Y1=225.32,Y2=127.98) #Parameter values#
...
ODE1<-function(times,state,parameters){ 
   ...
   dy<--(Y1*U1/4.5)*y-(Y2*U2/4.5)*y #equation
   res<-dy
   list(res)
}
...
glu<-c(55.482,26.802,16.311,13.706,13.248,7.54,1.278,2.694,3.08,0.757,2.465,2.673,3.183,7.631)
...
d <- ggplot() +
  ...
  xlab("Time (days)") + ylab("Glucose (mg/g)") +
  ggtitle("Brazil - WB1") +
  annotate("text", x = 2.5, y = 50, label = paste0("R² = ", round(R2, 4), ", MSE = ", round(MSE, 4),", RMSE =", round(RMSE, 4)))
...
d <- ggplot() +
  ...
  xlab("Time (days)") + ylab("Glucose (mg/g)") +
  ggtitle("Brazil - WB1") +
  annotate("text", x = 3.2, y = 58, label = paste0("R² = ", round(R2, 4), ", RMSE = ", round(RMSE, 4), ", p-value =", round(p_value, 4), ", D =", round(D, 4))) +
