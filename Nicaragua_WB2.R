...
state1<-c(y=20.684) #Initial state #
times1<-seq(0,6,1/10000) #6 days with 10000 intervals per day#
parameters1<-c(Y1=16.087,Y2=93.270) #Parameter values#
#Papalexandratou et al., 2019 Nicaragua <- Nicaragua - WB2#
...
ODE1<-function(times,state,parameters){ 
  ...
  dy<--(Y1*U1/0.1)*y-(Y2*U2/0.1)*y #equation 
  res<-dy
  list(res)
}
...
glu<-c(20.684,21.418,8.099,2.021,2.73,3.367,1.952)
...
d <- ggplot() +
  ...
  xlab("Time (days)") + ylab("Glucose (mg/g)") +
  ggtitle("Nicaragua - WB2") +
  annotate("text", x = 4, y = 50, label = paste0("R² = ", round(R2, 4), ", MSE = ", round(MSE, 4)))
...
d <- ggplot() +
  ...
  xlab("Time (days)") + ylab("Glucose (mg/g)") +
  ggtitle("Nicaragua - WB2") +
  annotate("text", x = 3.5, y = 50, label = paste0("R² = ", round(R2, 4), ", RMSE = ", round(RMSE, 4), ", p-value =", round(p_value, 4), ", D =", round(D, 4))) +
