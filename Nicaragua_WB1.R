...
state1<-c(y=20.293) #Initial state #
times1<-seq(0,5,1/10000) #5 days with 10000 intervals per day#
parameters1<-c(Y1=16.773,Y2=1145.05) #Parameter values#
...
ODE1<-function(times,state,parameters){ 
  ...
  dy<--(Y1*U1/17)*y-(Y2*U2/17)*y #equation 
  res<-dy
  list(res)
}
...
glu<-c(20.293,24.351,8.802,6.819,8.028,5.133)
...
d <- ggplot() +
  ...
  xlab("Time (days)") + ylab("Glucose (mg/g)") +
  ggtitle("Nicaragua - WB1") +
  annotate("text", x = 3, y = 50, label = paste0("R² = ", round(R2, 4), ", RMSE = ", round(RMSE, 4), ", p-value =", round(p_value, 4), ", D =", round(D, 4))) +
