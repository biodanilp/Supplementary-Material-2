...
state1<-c(y=56.909) # Initial state #
times1<-seq(0,6,1/10000) #6 days with 10000 intervals per day#
parameters1<-c(Y1=57.546,Y2=603.73) #Parameter values#
...
glu<-c(56.909,50.266,20.771,15.19,13.95,1.284,1.815)
...
d <- ggplot() +
  ...
  xlab("Time (days)") + ylab("Glucose (mg/g)") +
  ggtitle("Dominican Republic - WB1") +
  annotate("text", x = 3, y = 50, label = paste0("R² = ", round(R2, 4), ", MSE = ", round(MSE, 4)))
...
d <- ggplot() +
  ...
  xlab("Time (days)") + ylab("Glucose (mg/g)") +
  ggtitle("Dominican Republic - WB1") +
  annotate("text", x = 4, y = 55, label = paste0("R² = ", round(R2, 4), ", RMSE = ", round(RMSE, 4), ", p-value =", round(p_value, 4), ", D =", round(D, 4))) +
  ...
  scale_y_continuous(limits = c(0, 58), breaks = seq(0, 58, 10))   # y-axis boundaries and intervals between marks
