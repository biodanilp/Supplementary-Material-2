...
state1<-c(y=88.795) #Initial state#
times1<-seq(0,6,1/10000) #6 days with 10000 intervals per day#
parameters1<-c(Y1=80.118,Y2=840.54) #Parameter values#
#Lagunes Galvez et al., 2007 Dominican Republic <- Dominican Republic - WB1#
#doi:10.1016/j.ijfoodmicro.2006.10.041#
#Dominican Republic - WB1#
...
ODE1<-function(times,state,parameters){ 
  ...
  dy<--(Y1*U1/0.8)*y-(Y2*U2/0.8)*y #equation 
  res<-dy
  list(res)
}
...
fru<-c(88.795,70.992,40.877,28.034,29.008,12.621,12.09)
...
d<-ggplot() +
  ...
  ggtitle("Dominican Republic - WB1")
plot(d)
...
d <- ggplot() +
  ...
  xlab("Time (days)") + ylab("Fructose (mg/g)") +
  ggtitle("Dominican Republic - WB1") +
  annotate("text", x = 2, y = 89, label = paste0("R² = ", round(R2, 4), ", MSE = ", round(MSE, 4)))
...
d <- ggplot() +
  ...
  xlab("Time (days)") + ylab("Fructose (mg/g)") +
  ggtitle("Dominican Republic - WB1") +
  annotate("text", x = 4, y = 89, label = paste0("R² = ", round(R2, 4), ", RMSE = ", round(RMSE, 4), ", p-value =", round(p_value, 4), ", D =", round(D, 4))) +
  ...
  scale_y_continuous(limits = c(0, 89), breaks = seq(0, 89, 10))   # y-axis boundaries and intervals between marks
